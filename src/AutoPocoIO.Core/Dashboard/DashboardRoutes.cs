...
namespace AutoPocoIO.Dashboard
{
    ...
    public class DashboardRoutes
    {
        ...
        public DashboardRoutes()
        {
            ...
            Routes.Add("/Connectors/Delete/(?<id>.+)", HttpMethodType.POST, new CommandDispatcher<ConnectorsPage>((p, m) => p.Delete(m.GetString("id"))));
            Routes.Add("/Connectors/Connector/(?<id>.+)", HttpMethodType.GET, new RazorPageDispatcher<ConnectorForm>((p, m) => p.GetById(m.GetString("id"))));
            ...
            Routes.Add("/DataDictionary/Schema/(?<id>.+)", HttpMethodType.GET, new RazorPageDispatcher<SchemaPage>((p, m) => p.ListDbObjects(m.GetString("id"))));
            Routes.Add("/DataDictionary/Table/(?<id>.+)/(?<name>.+)", HttpMethodType.GET, new RazorPageDispatcher<ObjectDetailsPage>((p, m) => p.ListTableDetails(m.GetString("id"), m.GetString("name"))));
